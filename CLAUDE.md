...
| The tail of `/var/log/paas-cron.log` shows the dispatcher's current state, so a traceback at the end means it is broken now | The file is appended to **only when a cron actually fires**. With `crons` empty or all-disabled the dispatcher runs every minute, finds nothing and writes nothing, so the tail stays frozen on whatever last happened — on 2026-08-14 that was a `CalledProcessError` from **2026-07-31**, `mtime` 14 days stale, while the same `psql` command reproduced at exit 0 and `cron_dispatch`'s heartbeat was 0.07s old. Read `stat` and `task_heartbeat`, never the tail alone. The same trap applies to `/var/log/paas-watchdog.log`, which *is* written every 5 minutes and so is safe to tail |
| An app that is asleep cannot be serving traffic, so `state: asleep` with today's `last_seen` means a wake is failing | Frontend paths of a `kind: both` app are served by the static host without ever waking the backend (ARCHITECTURE §9c). `ping-pong` last ran 2026-08-05 and still answered `200` on 2026-08-14. `RouterName`/`ServiceName` in the proxy log say which side answered |
