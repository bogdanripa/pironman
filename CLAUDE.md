...
| The `ClientHost` on a fronted app's internal wake lines is `web`'s container IP, so `docker inspect` gives it | `docker inspect`'s `IPAddress` (and `.NetworkSettings.IPAddress`) is **IPv4 only**; the forward leaves `web` over IPv6, logging `fddf:6e69:23a7::e`. The §9c reconciliation recipe run with the IPv4 matches **zero** lines and looks like it failed — a false negative, not a fault. Read `.GlobalIPv6Address`. With the right address it was exact on 2026-08-19: 3 / 4 / 28 `fe-` 503s == `analytics_perf.err_server`, all 184 backend `500`s dropped |
