...
| The proxy access log is a file — `/var/log/traefik/access.log` in `coolify-proxy` | It is the container's **stdout**; read it with `docker logs coolify-proxy`, which is what `analytics.py` does over the Docker socket. No such file exists, and the `/data/coolify/proxy` mount holds config, not logs — so the wrong path returns **empty, exit 0**, which reads as "no traffic" rather than "wrong path" |
| A one-time host setup step documented in README has been applied to the box | Writing a required proxy flag into README does not put it on the proxy. `X-Pironman-Backend=keep` was still missing on 2026-08-06, a day after the commit that needed it shipped and deployed. Nothing warns: the proxy runs fine without it and the only symptom is quietly wrong numbers. Check `docker inspect` **and** whether the header appears in the log — a config edit needs a proxy restart, so the two disagree until then |
