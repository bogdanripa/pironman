...
| A sleeping app whose container has been `Exited` for days, while analytics show requests today, is failing to wake | `ping-pong` had not started since 5 Aug and was fine. Its 2 requests were bingbot on `/robots.txt` and `/`, matched by the **`fe-<id>` router** and served `200` by the static host — a fronted app's static paths never touch the backend. Read `RouterName` before concluding a wake failed |
| `ClientHost` in the proxy access log cannot identify which container made a request | It is the container's own **IPv6 ULA** on the `coolify` network, so it names it exactly — `fddf:6e69:23a7::e` was `web`, `::1` is the gateway (external traffic). It is unusable *in code* because it is recycled on redeploy, but decisive for an audit once you pin the redeploy times (ARCHITECTURE §10) |
| The checkout matching `origin/main` means the box is running everything that commit needs | A commit can add a **host-side prerequisite** the deploy cannot apply — `X-Pironman-Backend=keep` on the proxy shipped with 7871434 on 5 Aug and was still unapplied six days later. Nothing warns; `git` cannot see it. Verify host config against README's one-time setup, not against the image tag |
