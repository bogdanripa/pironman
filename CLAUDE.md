...
| A setup step written into README's "one-time host setup" was performed — the commit that documented it landed | Documentation is the *instruction*, never the receipt. `X-Pironman-Backend=keep` sat undone on `coolify-proxy` for two days after 7871434 documented it, silently leaving the analytics fallback partial. Read the running config (`docker inspect coolify-proxy --format '{{range .Args}}...'`), not the paragraph that asks for it |
| `ClientHost` is "not usable" for telling a static-host hop from a client request (ARCHITECTURE said so) | Not usable *as a stored key*, because the IP is recycled across redeploys. Read **live**, it is the sharpest one-off discriminator there is, and the only one that does not share an assumption with the router-name fallback. Watch the family: the proxy logs IPv6 on this network, `docker inspect .IPAddress` gives v4 — compare against `.GlobalIPv6Address` |
