...
| The outer-leg fix (`7871434`) shipped, so a fronted app's 5xx rate can be trusted now | Only where the proxy carries `--accesslog.fields.headers.names.X-Pironman-Backend=keep`, and **this box does not**: `docker inspect coolify-proxy` shows only `User-Agent` and `Cf-Connecting-Ip` kept, and every access-log line has an empty marker. The filter silently falls back to the router name, which cannot catch a cold wake's first probe. Read the flag off the container before believing any fronted app's 5xx number — the README documenting a setup step is not evidence it was ever applied |
