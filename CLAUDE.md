...
| README documents the five `--accesslog` flags as one-time setup, so the proxy has them | It has **four**. `X-Pironman-Backend=keep` was documented on 2026-08-05 and was still absent on 2026-08-18 and again on 2026-08-20 — fifteen days, unchanged. Documenting a host step does not perform it. Note a plain `docker restart` will not apply it either — `coolify-proxy` restarted on 2026-08-18 and came back with the same four flags, because a restart reuses the container's existing `Args`; the container has to be **recreated**. Every fronted sleeping app therefore still carries one phantom 5xx per wake |
| `analytics_perf` is under-counting — the proxy log holds ~2× its requests for every fronted app | It is **exact**. A fronted app logs two lines per request, both carrying the app's real Host, so grouping the log by `RequestHost` reads 2.0–2.7× and looks like catastrophic ingestion loss. Group by **`RouterName`**: the `fe-<app>` leg is the client's and is what analytics counts; the `http-0-<uuid>` leg is the static host's internal forward and is correctly dropped. Reconciled request-for-request on 2026-08-20 — `smartbill-mcp` 98 requests / 14 5xx / 15 4xx, `bt-gateway` 31/7/0, `wa-gateway` 136/0/0, each equal to the sum of its `fe-` lines |
| `docker inspect` gives the container IP to match against an access-log `ClientHost` | Not the address it prints first. Every `ClientHost` this proxy logs is **IPv6**, so `.IPAddress` (`web` = `10.0.1.14`) matches nothing while `.GlobalIPv6Address` (`fddf:6e69:23a7::e`) matches every line. Worse than useless in the §9c phantom-5xx recipe: zero matches reads as "no phantom errors, the rate is real", the inverse of the truth. `fddf:6e69:23a7::1` — the `coolify` network gateway — is the `ClientHost` of anything arriving from outside Docker, and unlike a container address it does not drift |
