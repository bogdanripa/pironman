...
| A big 5xx count in the proxy log means apps are failing | Most of it belongs to **no app**. An unrouted host matches `catchall@file`, which has no `ServiceName` and answers `503`. On 2026-09-01 that was **1,373 of 1,636** 5xx in 24h (84% of server errors, 43% of all access lines): 755 against the bare IP `5.12.126.43` and 618 with a forged `Host:` of `<md5>.<md5>.traefik.default` — Traefik's own default-rule domain, used as a fingerprint probe. One source sent 1,237, rotating forged bot UAs (GrokBot, Applebot, ClaudeBot) across `/.env`, `/.aws/credentials` and `169.254.169.254` SSRF paths. Restrict to `-coolify.bogdanripa.com` hosts **before** counting; the true client-visible figure that day was **1**. A third shape joins those two and is the one that misleads: a **real hostname pointed at this box by somebody else's DNS**. `ripa-setrarului.go.ro` resolves to `5.12.126.43`, the Pi's own public IP, and drew **286** `catchall@file` `503`s on 2026-09-07 — 20% of that day's 1,418 non-app 5xx, almost all one scanner on `/wp-login.php` and `/.git/config`. No `applications`/`service_applications` row and no container label carries that fqdn, so it is not an app whose route broke — but unlike a bare IP or an md5 host, it reads like one. Settle it on the registry, not on how the name looks |
| An `unhealthy` Coolify container found by the audit is a fault to chase down tonight | It may clear on its own. The 2026-08-28 power loss left `bootstrap/cache/*.php` at 0 bytes (§12) and `coolify` `unhealthy` with `FailingStreak 74` on 08-31; on 09-01 it read `healthy`, the cache files were non-zero and stamped `00:00`, and `/api/health` returned `200`. `bootstrap/cache` is on **no bind mount**, so it lives in the container filesystem and a recreate regenerates it — all four `coolify-*` containers were recreated at 00:00:47–53 (`Created` == `StartedAt`, `RestartCount` 0). Not an upgrade (still `4.3.14`, pulled 08-28) and nothing on the host did it, so it came from inside Coolify's scheduler. **The trigger is now proven** (2026-09-05): Coolify's own auto-update, `instance_settings.auto_update_frequency = '0 0 * * *'` with `is_auto_update_enabled = t`, firing on **UTC** (the container has no `TZ` and its `CheckForUpdatesJob` runs at `:00` of all 24 hours, matching `update_check_frequency = '0 * * * *'`). It recreated the same four at 00:00:44–50 on 09-05 carrying `4.3.14 → 4.3.17`. **The recreate, though, is version-gated — only the job is nightly** (corrected 2026-09-06, two routes): at `00:00:05` that morning `UpdateCoolifyJob` ran RUNNING → DONE in **376ms** in coolify's own scheduler log, and nothing was replaced — all four still read `Created` == `StartedAt` == `2026-09-05T00:00:44–50`, `RestartCount` 0, at 23:0x, on `4.3.17`, already the newest tag pulled on the box. So container age marks the **last version bump**, not last midnight; several days old is normal, and a night without a recreate is not the scheduler being dead. 09-01's recreate on an unchanged `4.3.14` stays **unexplained**. The job does **not** touch `coolify-proxy` or `coolify-sentinel`, so no nightly recreate will ever apply the missing `X-Pironman-Backend=keep` flag. The damage still survived **four days**, so re-check before escalating — and delete the empty caches rather than waiting on a recreate that may not come |
...
| The box rebooted overnight and `dmesg` shows `EXT4-fs orphan cleanup`, so it went down uncleanly — another power cut like 2026-08-28 | That line appears on **clean** boots too. It was there on the 2026-09-08T07:53:22Z boot, which followed a demonstrably orderly stop: at `07:50:50Z` the `_paas` postgres logged `received fast shutdown request` → `database system is shut down` and came back with `database system was shut down at 2026-09-08 07:50:50 UTC` (**not** "was not properly shut down"), while the api's uvicorn logged the whole `Shutting down` / `Application shutdown complete` / `Finished server process` sequence in the same second — two independent containers, both stopped in an orderly way 2m32s before boot. `dmesg` carried no ext4 **journal recovery** line and `grep -icE "error|fail"` over it returned **0**; `vcgencmd get_throttled` was `0x0`, `systemctl --failed` listed 0 units, and no zero-length files. So *why* it rebooted is unrecoverable — `last` is **not installed**, `rsyslog` is `inactive` with no `/var/log/syslog`, and `/var/log/journal` is **empty** because the journal lives on tmpfs at `/run/log/journal/<machine-id>`, so `journalctl --list-boots` only ever lists boot 0. Read the containers' json-file logs across the boot instead; they are on disk and survive it (ARCHITECTURE §12) |
...
| A custom domain is pointed at this box with a proxied CNAME to `<app>-coolify.bogdanripa.com` — it is the tidy answer and the platform's own docs said so | It **never reaches the origin**. That target is itself a *proxied* Cloudflare record, so the request is handed to the `bogdanripa.com` edge still carrying the custom hostname; that zone does not serve it, and Cloudflare answers `404` from inside. `cf-cache-status: DYNAMIC` and a `cf-ray` are present, so it reads exactly like an origin answering. Measured 2026-09-12: `gepetel.com` had precisely that CNAME and returned a bare 404 while the platform side was provably correct (`curl -H 'Host: gepetel.com' http://127.0.0.1/` on the box returned the app, on both the bundle and the backend leg). **The discriminator is the access log, and it is an absence**: a request Cloudflare forwarded carries `Cf-Connecting-Ip`, one it answered itself leaves no row at all — every `gepetel.com` row in 30 minutes was one of my own `ClientHost 10.0.1.1` probes. So before blaming routing here, ask whether the request arrived. The fix is an **A record to the box's public IP** (`5.12.126.43`, read from the box, not remembered), proxied, for a subdomain as much as for an apex — a CNAME only works to a **DNS-only** target, because the fault is the second proxy hop |
| A Traefik rule this repo generates is right if the unit tests pass and the labels land on the container | Neither says anything about whether **Traefik** accepts it. On 2026-09-12 the first custom domain wrote ``Host(`a`, `b`)`` — the **v2** spelling — and this box runs **v3.6.24**, where every matcher takes exactly one parameter: `unexpected number of parameters; got 2, expected one of [1]`. Everything upstream reported success — the row was written, `set_custom_labels` returned, the deploy finished, `docker inspect` showed the rule, 12 unit tests passed — and the only record of the failure was the proxy's own log. A rejected rule is **not** a degraded rule: Traefik drops that router and keeps the rest, so the app 404s with `router=(none)` and nothing else changes. **Scope it before calling it an outage**: `gepetel` was dark 09:40:41–09:42:30 (110s) while, in the same access-log window, `api-coolify` served `200`×6 on its own router and `wa-gateway` served `200`×3 through `fe-wa-gateway` — I had probed only gepetel hosts, read Traefik's bare `404 page not found` as entrypoint-wide, and told the user the platform was down. Two routes settled it: the access log's `RouterName` per host, and a throwaway `traefik:v3.6` where an invalid router 404s while a valid sibling still answers 200. The oracle for a rule string is Traefik itself — `tests/test_traefik_rules.py` — and it must assert **which router answered**, because the sibling trap (``Host(a) || Host(b) && Header(m)``, missing its parentheses) is valid syntax that silently un-scopes the backend |
| ARCHITECTURE §11's `fe-<id>` 5xx == wake lines == `err_server` identity closes a sleeping app's phantom-5xx audit | Both halves need a term the two earlier verifications had no instance of. **"Wake lines" is two sentence shapes, not one**: `INFO: wake <id>: served in …` is the success shape, and a wake whose retry budget runs out logs `WARNING: wake <id>: still failing after … ; passing the backend's <code> through` — which emits its `fe-` `503` all the same. On 2026-09-09 `bt-gateway` had 25 served + 4 failing = **29**, matching 29 `fe-` 5xx exactly; grepping only `served in` gives 25 and leaves 4 phantom 5xx looking unexplained. **And `err_server` == wakes + client-visible 5xx**, not == wakes: that day `bt-gateway` read 33 = 29 + the 4 client-visible `502`s those failed wakes passed through (`sablier 0.01–0.04s`, so nothing was started — the backend was up and answering `502` itself, §9c). 2026-08-12 and 2026-08-27 both happened to have **zero** client-visible 5xx, so the term was invisible and the identity looked simpler than it is. `revolut-mcp` closed exactly (5/5/5); `smartbill-mcp` was off by one (45 `fe-` vs 44 wake lines), unexplained |
| The wake-log route is the redeploy-proof one — ARCHITECTURE §11 says it needs neither `web`'s address nor the trailing `?`, "so it survives a redeploy inside the window" | The **arithmetic** survives; the **evidence** does not. Wake lines live only in `web`'s container log, and a deploy **recreates** that container — Docker deletes the removed container's json-file log with it, so the older wake lines are gone, not merely awkward to reach. On 2026-09-10 `web` was recreated at `11:31:03Z`, and a nominal-24h reconciliation read as a **4× deficit**: 153 internal-leg `503`s (`smartbill-mcp` 70, `revolut-mcp` 46, `bt-gateway` 37) against **39** available wake lines, leaving 114 phantom 5xx looking unexplained. Clamped to `11:31:03Z`–`23:03Z` it closes — `revolut-mcp` 9/9, `bt-gateway` 10/10, `smartbill-mcp` 23/20 (residual 3, unexplained). Three routes fixed the window: `Created` == `StartedAt` with `RestartCount` 0; the `web`/`finished` row at `11:30:48` in coolify-db `application_deployment_queues`; and the log's own first line being uvicorn's `Started server process [1]`. That the old log is *destroyed* rather than orphaned is checkable directly — `/var/lib/docker/containers` held 23 dirs against 23 containers, exactly one of them `web`'s. So read `web`'s `StartedAt` before the wake count exactly as before its address, and treat a deficit that scales with how far the window predates the redeploy as the window, not a fault |
